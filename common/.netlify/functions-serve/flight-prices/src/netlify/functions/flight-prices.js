...
// ../netlify/functions/flight-prices.js
...
var getFlightPricesForDates = async (origin2, destination2, dates) => {
  const pricePromises = dates.map(async (date) => {
    try {
      ...
      console.log(`Amadeus API response for ${origin2} to ${destination2} on ${date}:`, JSON.stringify(response, null, 2));
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (prices.length === 0) {
      ...
      const basePrice2 = 500 + Math.random() * 200;
      const mockPrices = dates.map((date) => {
        const randomVariation = Math.random() * 100 - 50;
        return {
          date,
          price: Math.round(basePrice2 + randomVariation)
        };
      });
...
//# sourceMappingURL=flight-prices.js.map
