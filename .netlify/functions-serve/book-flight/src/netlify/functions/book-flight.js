...
// netlify/functions/book-flight.js
var book_flight_exports = {};
__export(book_flight_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(book_flight_exports);
var import_amadeus = __toESM(require_amadeus());
var amadeus;
var getConfig = async () => {
...
var handler = async (event, context) => {
  ...
  try {
    ...
    if (!bookingData.flightOffer || !bookingData.travelers || !bookingData.contacts) {
      return {
        statusCode: 400,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          error: "Missing required booking data: flightOffer, travelers, and contacts are required"
        })
      };
    }
    console.log("Processing booking with data:", {
      flightOffer: bookingData.flightOffer.id,
      travelers: bookingData.travelers.length,
      contacts: bookingData.contacts.length
    });
...
//# sourceMappingURL=book-flight.js.map
