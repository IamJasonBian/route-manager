...
async function archiveStore(srcStore, dstStore, cutoff) {
  console.log(`\n--- ${srcStore} → ${dstStore} ---`);

  const keys = await listBlobs(srcStore);
  const oldKeys = keys.filter(k => k < cutoff).sort();

  console.log(`Total blobs in ${srcStore}: ${keys.length}`);
  ...
  for (const key of oldKeys) {
    try {
      ...
      await request('PUT', `${BLOBS_BASE}/${SITE_ID}/${dstStore}/${encodeURIComponent(key)}`, data);
      await request('DELETE', `${BLOBS_BASE}/${SITE_ID}/${srcStore}/${encodeURIComponent(key)}`);
      moved++;
...
async function main() {
  ...
  console.log(`Archiving blobs older than ${RETENTION_DAYS} days (cutoff: ${cutoff})`);

  let totalMoved = 0, totalFailed = 0;
  for (const { src, dst } of STORE_PAIRS) {
    const { moved, failed } = await archiveStore(src, dst, cutoff);
    totalMoved += moved;
    totalFailed += failed;
  }

  console.log(`\nDone: ${totalMoved} archived, ${totalFailed} failed`);
